...
| [`herdr-rpiv-ask-user-question.ts`](extensions/herdr-rpiv-ask-user-question.ts) | `herdr:blocked` bridge | 在 `ask_user_question` 工具执行期间将 Herdr Agent 状态切换为 `blocked`，结束或会话关闭时恢复，避免第三方问答弹窗一直显示为 `working`。 | Herdr 及其 Pi integration；`@juicesharp/rpiv-ask-user-question`。 |
...
`herdr-rpiv-ask-user-question.ts` 监听 Pi 的 `ask_user_question` 工具执行生命周期，并向 Herdr 官方 Pi integration 使用的共享事件总线发送成对的阻塞开始/结束事件。它不修改 Herdr 管理的 integration 文件，也不修改问答插件；缺少任一配套组件时不会产生可见效果。安装或更新 Herdr integration 后无需重新应用此扩展。
