...
| [`herdr-answer-studio`](extensions/herdr-answer-studio/index.ts) | `/answer`、自动触发、`herdr:blocked` bridge | 集成固定版本的 Answer Studio，在最终 Assistant 回复看起来要求用户输入时直接打开回答界面，并同步 Herdr 侧边栏状态。 | Herdr 及其 Pi integration；不要再单独加载 `@petechu/pi-answer-studio`。 |
| [`set-pane-title.ts`](extensions/set-pane-title.ts) | `/set-pane-title [文字]` | 将 Herdr pane 左上角的 agent label 临时设为“当前 agent - 输入文字”；重复调用时最后一次生效，并在切换 session 或退出时清除。 | Herdr 及其 Pi integration；不带参数时需要交互式 UI。 |
...
| [`session-digest.ts`](extensions/session-digest.ts) | `/digest [all\|ai\|tool\|user]` | 在分页 overlay 中筛选浏览当前 session branch 的用户、Assistant 与工具消息，显示时间、相邻消息间隔、工具调用总数和用户轮次，并可展开滚动查看详情。 | 仅 TUI；每种筛选最多展示最近 500 条消息，并限制单条展示长度。 |
...
`session-digest.ts` 改编自 `disler/pi-vs-claude-code` 在 commit `32dfe122cb6d444e91c68b32597274a725d81fa3` 中的实现，并保留上游 MIT 许可证于 [`extensions/session-digest.LICENSE`](extensions/session-digest.LICENSE)。收录版本改为当前 Pi package imports，移除了原项目的主题映射依赖，并补充内容上限、终端控制字符清理、敏感工具参数脱敏、可配置按键支持和有界分页显示。`/digest` 默认展示全部消息，也可通过 `ai`、`tool` 或 `user` 参数筛选；底部统计当前 branch 中的 Tool 结果数（作为调用次数）和用户消息轮次。界面只使用当前 Pi 主题的语义颜色，并提高了暗色主题下预览文字、选中项和边框的对比度。所有时间均按本机时区以 `00:00:00`–`23:59:59` 显示。
...
`herdr-answer-studio` 是一个集成入口：它加载固定版本的 `@petechu/pi-answer-studio`，并把手工 `/answer` 与自动触发统一到同一个受控 handler。Agent 完全结束后，问号或常见中英文询问句式会直接打开 Answer Studio，不会再把字面量 `/answer` 作为用户消息发送给模型。无论自动还是手工进入，回答界面存续期间 Herdr 都显示 `blocked`；提交并启动后续 Agent 时恢复为 `working`，取消、空提取或异常时解除阻塞，随后由 Herdr 恢复为 `done` 或 `idle`。同一 Assistant 消息每个 runtime 只触发一次。使用该入口时不要再把原始 Answer Studio 作为另一个 extension 单独加载，否则会产生重复命令。它不修改 Herdr 管理的 integration 文件；缺少 Herdr integration 时不会产生侧边栏状态效果。安装或更新 Herdr integration 后无需重新应用此扩展。
