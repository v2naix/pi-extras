...
| [`code-notebook-teacher`](skills/code-notebook-teacher/SKILL.md) | 读取已有 Teach skill 的教学方法，基于源码上下文和具体问题在单个 Jupyter Notebook 中生成讲解、可执行实验、练习和学习进度。 | 仅显式调用 `/skill:code-notebook-teacher`；必须指定 `.ipynb` 路径，执行代码单元需要已有 Teach skill、对应内核和项目环境。 |
...
`code-notebook-teacher` 在每次教学前读取环境中已有的 `teach` skill，复用其中的 mission、最近发展区、检索练习和反馈回路等教学方法，再将其目录式存储模型映射到一个 `.ipynb`。两个 skill 名称不同，可以同时启用；前者不会生成 Teach 默认的 HTML 与课程目录。
...
| --- | --- | --- | --- |
| [`compact-footer`](extensions/compact-footer/index.ts) | TUI footer | 用紧凑的一行显示目录、Git 分支、会话名、扩展状态、上下文占用、模型和 thinking level。 | 仅 TUI；建议使用 Nerd Font。 |
| [`copy-all`](extensions/copy-all/index.ts) | `/copy-all` | 将当前会话分支中的全部用户与 Assistant 消息复制到系统剪贴板。 | macOS `pbcopy`。 |
| [`copy-code`](extensions/copy-code/index.ts) | `/copy-code [edit]`、`Ctrl+Alt+C` | 从最近 10 条含 fenced code block 的 Assistant 回复中选择并复制代码；`edit` 在 Pi 内置编辑器中编辑后复制。 | 优先使用系统剪贴板工具，缺失时使用有长度上限的 OSC 52。 |
...
| [`dotfiles-workflow`](extensions/dotfiles-workflow/index.ts) | `/dw [check\|status\|review\|verify\|doctor\|applyLocal\|applySource\|help]` | Dotfiles Shell 核心的轻量诊断适配层；仅在已识别的 Dotfiles 源内委托公开命令，显示简洁状态，并阻止直接写入该仓库的 `.git` 元数据。 | 对应 Dotfiles 源及其 `scripts/dotfiles`；不会自动 apply、提交、推送、导入漂移或修改 Pi 配置。 |
...
| [`set-pane-title`](extensions/set-pane-title/index.ts) | `/set-pane-title [文字]`、自动命名 | 完成首轮对话后调用当前模型生成一次不超过 20 个字符的默认标题，并将 Herdr pane 左上角的 agent label 设为“agent logo - 标题”（pi 显示 π，claude 显示 ✱，其他 agent 用名称）；之后可用命令手动覆盖，切换 session 或退出时清除。 | Herdr 及其 Pi integration；自动命名会额外调用一次当前模型，不带命令参数时需要交互式 UI。 |
