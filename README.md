...
| [`session-replay.ts`](extensions/session-replay.ts) | `/replay` | 在分页 overlay 中浏览当前 session branch 的用户、Assistant 与工具消息，显示 24 小时制时间和相邻消息间隔，并可展开滚动查看详情。 | 仅 TUI；最多读取最近 500 条消息，并限制单条展示长度。 |
...
`session-replay.ts` 改编自 `disler/pi-vs-claude-code` 在 commit `32dfe122cb6d444e91c68b32597274a725d81fa3` 中的实现，并保留上游 MIT 许可证于 [`extensions/session-replay.LICENSE`](extensions/session-replay.LICENSE)。收录版本改为当前 Pi package imports，移除了原项目的主题映射依赖，并补充内容上限、终端控制字符清理、敏感工具参数脱敏、可配置按键支持和有界分页显示。所有时间均按本机时区以 `00:00:00`–`23:59:59` 显示。
