...
| [`herdr-answer-studio`](extensions/herdr-answer-studio/index.ts) | `/answer`、自动触发、`herdr:blocked` bridge | 内置本地维护的 Answer Studio fork，在最终 Assistant 回复看起来要求用户输入时直接打开回答界面，并同步 Herdr 侧边栏状态。 | Herdr 及其 Pi integration；不要再单独加载其他 Answer Studio 扩展。 |
