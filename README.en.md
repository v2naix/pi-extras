...
| [`session-digest.ts`](extensions/session-digest.ts) | `/digest [all\|ai\|tool\|user]` | Filters and browses user, assistant, and tool messages from the current session branch in a paged overlay, with timestamps, elapsed times, total tool calls, user turns, and scrollable details. | TUI only; displays at most the latest 500 messages per filter and limits displayed content per entry. |
...
`session-digest.ts` is adapted from the implementation in `disler/pi-vs-claude-code` at commit `32dfe122cb6d444e91c68b32597274a725d81fa3`. Its upstream MIT license is retained in [`extensions/session-digest.LICENSE`](extensions/session-digest.LICENSE). This collected version uses the current Pi package imports, removes the source project's theme-map dependency, and adds content bounds, terminal-control sanitization, sensitive tool-argument redaction, configurable keybindings, and bounded pagination. `/digest` shows all messages by default and accepts `ai`, `tool`, or `user` filters. Its footer counts tool-result messages as calls and user messages as user turns across the current branch. The UI uses only semantic colors from the active Pi theme and increases contrast for previews, selections, and borders on dark themes. Times use the machine's local time zone and always display as `00:00:00`–`23:59:59`.
