...
| [`dotfiles-workflow`](extensions/dotfiles-workflow/index.ts) | `/df-check`, `/df-status`, `/df-review`, `/df-verify`, `/df-doctor`, `/df-help` | A thin diagnostic adapter for the Dotfiles Shell core. It delegates only public commands inside a recognized source, shows compact status, and blocks direct writes to that repository's `.git` metadata. | The matching Dotfiles source and its `scripts/dotfiles`; never auto-applies, commits, pushes, imports drift, or changes Pi configuration. |
| [`dotfiles-publish`](extensions/dotfiles-publish/index.ts) | `/dotfiles-publish` | Shows Git, chezmoi, and remote state in the Pi TUI, then uses staged confirmations to review, apply, test, scan, commit, reconcile, and finally push. | The matching Dotfiles source; pushes only from `main` to `origin/main`, never force-pushes, and runs repository tests and security gates. |
...
### Interactive Dotfiles publishing

`dotfiles-publish` is a mutating interactive orchestrator kept separate from the read-only `dotfiles-workflow` diagnostic extension. It shows local state first, fetches only after confirmation, and then lets the operator choose “publish to remote,” “apply only,” or “check only.” The publish path performs chezmoi review/apply, repository tests, a full security scan, commit, remote reconciliation, and the final push. Commit messages, target imports or overwrites, fast-forwards, rebases, and pushes each require a separate decision.

When target drift is found, the operator can review each diff and corresponding source path, then import, skip, or stop without running chezmoi commands manually. Regular changes are imported with secret-checked `chezmoi add`; local deletions require another confirmation before `chezmoi forget` reflects them in source, and replacing a template while removing its template attribute also requires an extra confirmation. Unresolved drift stops publishing and preserves local changes, while an explicit choice can instead overwrite the listed targets from source. A remote divergence creates a `safety/diverged-*` branch before an explicitly confirmed rebase. Conflicts, failed tests, failed security gates, failed hooks, and non-`main` branches stop the workflow. The extension always uses `git push origin HEAD:main`, with no force-push or hook-bypass path. If a fast-forward or rebase changes HEAD, it repeats review, apply, and validation.
