...
| [`copy-code.ts`](extensions/copy-code.ts) | `/copy-code [edit]`, `Ctrl+Alt+C` | Select and copy fenced code blocks from the last 10 Assistant responses containing code; `edit` opens Pi's built-in editor before copying. | Uses native clipboard tools first, then size-bounded OSC 52. |
