...
| [`herdr-answer-studio`](extensions/herdr-answer-studio/index.ts) | `/answer`, automatic invocation, `herdr:blocked` bridge | Bundles a locally maintained Answer Studio fork, directly opens the answer UI when the final assistant response appears to request input, and synchronizes the Herdr sidebar. | Herdr with its Pi integration; do not load another Answer Studio extension separately. |
